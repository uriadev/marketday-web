...
- **Launch offer.** Vendors created before the API's `LAUNCH_OFFER_JOINED_BEFORE` get 50% off their first six monthly bills. The API applies the Stripe coupon at checkout, and `billingOverview.launchOfferApplies` says when it will. `VendorPlan` then shows the half price. The marketing mentions all read `launchOffer` in `src/data/pricing.ts`. Set its `active` to false when the launch ends.
